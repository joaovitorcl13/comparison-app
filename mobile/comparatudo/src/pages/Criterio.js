...
function Criterio() {
    ...
    const [novoCriterio, setNovoCriterio] = useState(null);
    const [alterarCriterio, setAlterarCriterio] = useState(null);
    ...
    async function handleExcluir(obj) {
        try {
            ...
            const response = await api.delete(`/criterio/${obj}`);
    ...
    async function handleNovo() {
        ...
        setNovoCriterio(null);
        try {

            const response = await api.post('/criterio', { name: nome, desc: descricao });
        } catch (error) {
            Alert.alert(
                'Falha',
                `${error}`,
    ...
    async function handleAlterar() {
        ...
        setAlterarCriterio(null);
        try {

            const response = await api.put(`/criterio/${itemAlterar.cod}`, { name: nome, desc: descricao });
        } catch (error) {
            Alert.alert(
                'Falha',
                `${error}`,
        ...
        <SafeAreaView style={style.container}>
            ...
            <View style={style.card}>
                <View style={style.cabe}>
                    ...
                    <Text style={style.text}>Critérios</Text>

                </View>
                <ScrollView>
                    {itens.length === 0 ? <Text style={style.buttonText}> Nenhum Criterio cadastrado</Text>
                        : (
                            renderItem()
                ...
                <View>
                    <TouchableOpacity onPress={setNovoCriterio} style={style.button}>
    ...
    function setAlterar(obj) {
        ...
        setAlterarCriterio(true)
    }
    function viewNovoCriterio() {
        return (
            <SafeAreaView style={style.novoContainer}>
                <Text style={style.text}>Novo Critério</Text>
                <TextInput
                    placeholder="Digite Nome do novo Criterio"
                    style={style.input}
                    value={nome}
                    onChangeText={setNome}
                />
                <TextInput
                    placeholder="Digite a descrição do novo Criterio"
                    style={style.input}
                    value={descricao}
                ...
                <View style={{ flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity onPress={() => { setNovoCriterio(null) }} style={style.buttonNovo}>
    ...
    function viewAlterarCriterio() {
        return (
            <SafeAreaView style={style.novoContainer}>
                <Text style={style.text}>Alterar Critério #{itemAlterar.cod}</Text>
                <TextInput
                ...
                <View style={{ flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity onPress={() => { setAlterarCriterio(null) }} style={style.buttonNovo}>
                        <Text style={style.buttonText}>Cancelar</Text>
                    </TouchableOpacity>
